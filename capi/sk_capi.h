...
typedef enum {
  ...
  SK_PATH_DIRECTION_CCW, // counter-clockwise direction for adding closed
                         // contours
} sk_path_direction_t;

typedef enum {
  SK_PATH_FILLTYPE_WINDING, // Specifies that "inside" is computed by a non-zero
                            // sum of signed edge crossings
  SK_PATH_FILLTYPE_EVENODD, // Specifies that "inside" is computed by an odd
                            // number of edge crossings
  SK_PATH_FILLTYPE_INVERSE_WINDING, // Same as Winding, but draws outside of the
                                    // path, rather than inside
  SK_PATH_FILLTYPE_INVERSE_EVENODD, // Same as EvenOdd, but draws outside of the
                                    // path, rather than inside
} sk_path_fill_type_t;
...
typedef enum {
  ...
  SK_PATH_EFFECT_1D_STYLE_MORPH, // transform each point, and turn lines into
                                 // curves
...
typedef enum {
  ...
  SK_PATH_EFFECT_TRIM_MODE_INVERTED, // return the complement/subset paths
                                     // [0,start] + [stop,1]
...
typedef enum {
  ...
  SK_BLEND_MODE_HUE,        // hue of source with saturation and luminosity of
                            // destination
  SK_BLEND_MODE_SATURATION, // saturation of source with hue and luminosity of
                            // destination
  SK_BLEND_MODE_COLOR,      // hue and saturation of source with luminosity of
                            // destination
  SK_BLEND_MODE_LUMINOSITY, // luminosity of source with hue and saturation of
                            // destination
...
typedef enum {
  ...
  SK_HIGH_CONTRAST_CONFIG_INVERT_STYLE_LAST =
      SK_HIGH_CONTRAST_CONFIG_INVERT_STYLE_INVERT_LIGHTNESS,
...
typedef enum {
  ...
  SK_COLOR_TYPE_RGB_565, // pixel with 5 bits red, 6 bits green, 5 bits blue, in
                         // 16-bit word
  SK_COLOR_TYPE_ARGB_4444, // pixel with 4 bits for alpha, red, green, blue; in
                           // 16-bit word
  SK_COLOR_TYPE_RGBA_8888, // pixel with 8 bits for red, green, blue, alpha; in
                           // 32-bit word
  SK_COLOR_TYPE_RGB_888X,  // pixel with 8 bits each for red, green, blue; in
                           // 32-bit word
  SK_COLOR_TYPE_BGRA_8888, // pixel with 8 bits for blue, green, red, alpha; in
                           // 32-bit word
  SK_COLOR_TYPE_RGBA_1010102, // 10 bits for red, green, blue; 2 bits for alpha;
                              // in 32-bit word
  SK_COLOR_TYPE_BGRA_1010102, // 10 bits for blue, green, red; 2 bits for alpha;
                              // in 32-bit word
  SK_COLOR_TYPE_RGB_101010X, // pixel with 10 bits each for red, green, blue; in
                             // 32-bit word
  SK_COLOR_TYPE_BGR_101010X, // pixel with 10 bits each for blue, green, red; in
                             // 32-bit word
  SK_COLOR_TYPE_BGR_101010X_XR, // pixel with 10 bits each for blue, green, red;
                                // in 32-bit word, extended range
  SK_COLOR_TYPE_BGRA_10101010_XR, // pixel with 10 bits each for blue, green,
                                  // red, alpha; in 64-bit word, extended range
  SK_COLOR_TYPE_RGBA_10x6,        // pixel with 10 used bits (most significant)
                           // followed by 6 unused bits for red, green, blue,
                           // alpha; in 64-bit word
  SK_COLOR_TYPE_GRAY_8,        // pixel with grayscale level in 8-bit byte
  SK_COLOR_TYPE_RGBA_F16_NORM, // pixel with half floats in [0,1] for red,
                               // green, blue, alpha; in 64-bit word
  SK_COLOR_TYPE_RGBA_F16, // pixel with half floats for red, green, blue, alpha;
                          // in 64-bit word
  SK_COLOR_TYPE_RGBA_F16F16F16x, // pixel with half floats for red, green, blue;
                                 // in 64-bit word
  SK_COLOR_TYPE_RGBA_F32, // pixel using C float for red, green, blue, alpha; in
                          // 128-bit word
  ...
  SK_COLOR_TYPE_R16G16_UNORM, // pixel with a little endian uint16_t for red and
                              // green
  SK_COLOR_TYPE_R16G16B16A16_UNORM, // pixel with a little endian uint16_t for
                                    // red, green, blue and alpha

  SK_COLOR_TYPE_SRGBA_8888, // pixel with 8 bits for red, green, blue, alpha; in
                            // 32-bit word with conversion between sRGB and
                            // linear space
...
typedef enum {
  SK_IMAGE_CACHING_HINT_ALLOW, // allows internally caching decoded and copied
                               // pixels
  SK_IMAGE_CACHING_HINT_DISALLOW, // disallows internally caching decoded and
                                  // copied pixels
...
typedef enum {
  SK_FONT_METRICS_FLAG_UNDERLINE_THICKNESS_IS_VALID =
      1 << 0, // set if underlineThickness is valid
  SK_FONT_METRICS_FLAG_UNDERLINE_POSITION_IS_VALID =
      1 << 1, // set if underlinePosition is valid
  SK_FONT_METRICS_FLAG_STRIKEOUT_THICKNESS_IS_VALID =
      1 << 2, // set if strikeoutThickness is valid
  SK_FONT_METRICS_FLAG_STRIKEOUT_POSITION_IS_VALID =
      1 << 3, // set if strikeoutPosition is valid
  SK_FONT_METRICS_FLAG_BOUNDS_INVALID =
      1 << 4, // set if top, bottom, xMin, xMax invalid
} sk_font_metrics_flags_t;

typedef struct {
  uint32_t flags; // FontMetricsFlags indicating which metrics are valid
  float top;      // greatest extent above origin of any glyph bounding box,
  ...
  float bottom;   // greatest extent below origin of any glyph bounding box,
  ...
  float xMin; // greatest extent to left of origin of any glyph bounding box,
              // typically negative; deprecated with variable fonts
  float xMax; // greatest extent to right of origin of any glyph bounding box,
              // typically positive; deprecated with variable fonts
  float
      xHeight; // height of lower-case 'x', zero if unknown, typically negative
  float capHeight; // height of an upper-case letter, zero if unknown, typically
                   // negative
  float underlineThickness; // underline thickness
  float underlinePosition; // distance from baseline to top of stroke, typically
                           // positive
  float strikeoutThickness; // strikeout thickness
  float strikeoutPosition;  // distance from baseline to bottom of stroke,
                            // typically negative
} sk_font_metrics_t;
...
typedef enum {
  SK_TILE_MODE_CLAMP, // Replicate the edge color if the shader draws outside of
                      // its original bounds
  SK_TILE_REPEAT,     // Repeat the shader's image horizontally and vertically
  SK_TILE_MIRROR,     // Repeat the shader's image horizontally and vertically,
                  // alternating mirror images so that adjacent images always
                  // seam
  SK_TILE_DECAL, // Only draw within the original domain, return
...
SK_C_API gr_backendrendertarget_t *
...
SK_C_API void
gr_backendrendertarget_delete(gr_backendrendertarget_t *rendertarget);
SK_C_API gr_direct_context_t *
...
SK_C_API void gr_direct_context_flush_and_submit(gr_direct_context_t *context,
                                                 bool syncCPU);
SK_C_API void gr_direct_context_release_resources_and_abandon_context(
    gr_direct_context_t *context);
SK_C_API void gr_direct_context_reset(gr_direct_context_t *context);
SK_C_API void
gr_direct_context_reset_gl_texture_bindings(gr_direct_context_t *context);
...
SK_C_API void sk_canvas_clip_rect_with_operation(sk_canvas_t *t,
                                                 const sk_rect_t *crect,
                                                 sk_clip_op_t op, bool doAA);
SK_C_API void sk_canvas_concat(sk_canvas_t *canvas, const sk_matrix_t *matrix);
SK_C_API void sk_canvas_draw_arc(sk_canvas_t *canvas, const sk_rect_t *oval,
                                 float startAngle, float sweepAngle,
                                 bool useCenter, const sk_paint_t *paint);
...
SK_C_API void sk_canvas_draw_image_rect(
    sk_canvas_t *canvas, const sk_image_t *cimage, const sk_rect_t *csrcR,
    const sk_rect_t *cdstR, const sk_sampling_options_t *samplingOptions,
    const sk_paint_t *cpaint, sk_src_rect_constraint_t constraint);
SK_C_API void sk_canvas_draw_line(sk_canvas_t *canvas, float x0, float y0,
                                  float x1, float y1, const sk_paint_t *cpaint);
SK_C_API void sk_canvas_draw_oval(sk_canvas_t *canvas, const sk_rect_t *crect,
                                  const sk_paint_t *cpaint);
SK_C_API void sk_canvas_draw_paint(sk_canvas_t *canvas,
                                   const sk_paint_t *cpaint);
SK_C_API void sk_canvas_draw_path(sk_canvas_t *canvas, const sk_path_t *cpath,
                                  const sk_paint_t *cpaint);
SK_C_API void sk_canvas_draw_point(sk_canvas_t *canvas, float x, float y,
                                   const sk_paint_t *cpaint);
SK_C_API void sk_canvas_draw_points(sk_canvas_t *canvas,
                                    sk_point_mode_t pointMode, size_t count,
                                    const sk_point_t points[],
                                    const sk_paint_t *cpaint);
SK_C_API void sk_canvas_draw_rect(sk_canvas_t *canvas, const sk_rect_t *crect,
                                  const sk_paint_t *cpaint);
SK_C_API void sk_canvas_draw_round_rect(sk_canvas_t *canvas,
                                        const sk_rect_t *crect, float rx,
                                        float ry, const sk_paint_t *cpaint);
SK_C_API void sk_canvas_draw_simple_text(sk_canvas_t *canvas, const void *text,
                                         size_t byte_length,
                                         sk_text_encoding_t encoding, float x,
                                         float y, const sk_font_t *cfont,
                                         const sk_paint_t *cpaint);

SK_C_API void sk_canvas_draw_string(sk_canvas_t *canvas, const char str[],
                                    float x, float y, const sk_font_t *cfont,
                                    const sk_paint_t *cpaint);

SK_C_API void sk_canvas_draw_text_blob(sk_canvas_t *canvas,
                                       sk_text_blob_t *text, float x, float y,
                                       const sk_paint_t *paint);
SK_C_API bool sk_canvas_get_local_clip_bounds(sk_canvas_t *canvas,
                                              sk_rect_t *cbounds);
SK_C_API int sk_canvas_get_save_count(sk_canvas_t *canvas);
SK_C_API void sk_canvas_get_total_matrix(sk_canvas_t *canvas,
                                         sk_matrix_t *matrix);
SK_C_API bool sk_canvas_is_clip_empty(sk_canvas_t *canvas);
SK_C_API bool sk_canvas_is_clip_rect(sk_canvas_t *canvas);
SK_C_API bool sk_canvas_quick_reject_path(sk_canvas_t *canvas,
                                          const sk_path_t *path);
SK_C_API bool sk_canvas_quick_reject_rect(sk_canvas_t *canvas,
...
SK_C_API int sk_canvas_save_layer(sk_canvas_t *canvas, const sk_rect_t *crect,
                                  const sk_paint_t *cpaint);
SK_C_API int sk_canvas_save_layer_alpha(sk_canvas_t *canvas,
                                        const sk_rect_t *crect, uint8_t alpha);
SK_C_API void sk_canvas_scale(sk_canvas_t *canvas, float sx, float sy);
SK_C_API void sk_canvas_set_matrix(sk_canvas_t *canvas,
...
sk_colorfilter_new_color_matrix(const float array[20]);
SK_C_API sk_color_filter_t *
sk_colorfilter_new_compose(sk_color_filter_t *outer, sk_color_filter_t *inner);
SK_C_API sk_color_filter_t *
sk_colorfilter_new_high_contrast(const sk_high_contrast_config_t *config);
SK_C_API sk_color_filter_t *sk_colorfilter_new_lighting(sk_color_t mul,
                                                        sk_color_t add);
...
SK_C_API sk_data_t *sk_encode_jpeg(gr_direct_context_t *ctx,
                                   const sk_image_t *img, int quality);

// ===== Functions from include/encode/SkPngEncoder.h =====
SK_C_API sk_data_t *sk_encode_png(gr_direct_context_t *ctx,
                                  const sk_image_t *img, int compressionLevel);

// ===== Functions from include/encode/SkWebpEncoder.h =====
SK_C_API sk_data_t *sk_encode_webp(gr_direct_context_t *ctx,
                                   const sk_image_t *img, float quality,
                                   bool lossy);
...
SK_C_API void sk_font_get_xpos(const sk_font_t *font, const uint16_t glyphs[],
                               int count, float xpos[], float origin);
SK_C_API float sk_font_measure_text(const sk_font_t *font, const void *text,
                                    size_t byteLength,
                                    sk_text_encoding_t encoding,
                                    sk_rect_t *bounds, const sk_paint_t *paint);
SK_C_API sk_font_t *sk_font_new_with_values(sk_typeface_t *typeface, float size,
...
SK_C_API int sk_font_text_to_glyphs(const sk_font_t *font, const void *text,
                                    size_t byteLength,
                                    sk_text_encoding_t encoding,
                                    uint16_t glyphs[], int maxGlyphCount);
SK_C_API uint16_t sk_font_unichar_to_glyph(const sk_font_t *font,
                                           int32_t unichar);
SK_C_API void sk_font_unichars_to_glyphs(const sk_font_t *font,
                                         const int32_t *unichars, int count,
                                         uint16_t *glyphs);
SK_C_API void sk_font_glyph_widths(const sk_font_t *font,
                                   const uint16_t *glyphs, int count,
                                   float *widths);

// ===== Functions from include/core/SkFontMgr.h =====
SK_C_API int sk_fontmgr_count_families(sk_font_mgr_t *fontmgr);
SK_C_API sk_typeface_t *sk_fontmgr_create_from_data(sk_font_mgr_t *fontmgr,
                                                    sk_data_t *data, int index);
SK_C_API void sk_fontmgr_get_family_name(sk_font_mgr_t *fontmgr, int index,
                                         sk_string_t *familyName);
SK_C_API sk_font_style_set_t *sk_fontmgr_match_family(sk_font_mgr_t *fontmgr,
                                                      const char *familyName);
SK_C_API sk_typeface_t *sk_fontmgr_match_family_style(sk_font_mgr_t *fontmgr,
                                                      const char *familyName,
                                                      sk_font_style_t *style);
SK_C_API sk_typeface_t *sk_fontmgr_match_family_style_character(
...
SK_C_API sk_typeface_t *
sk_fontstyleset_create_typeface(sk_font_style_set_t *fss, int index);
SK_C_API int sk_fontstyleset_get_count(sk_font_style_set_t *fss);
SK_C_API void sk_fontstyleset_get_style(sk_font_style_set_t *fss, int index,
                                        sk_font_style_t *fs,
                                        sk_string_t *style);
SK_C_API sk_typeface_t *sk_fontstyleset_match_style(sk_font_style_set_t *fss,
                                                    sk_font_style_t *style);
...
sk_fontstyle_get_slant(const sk_font_style_t *fs);
...
SK_C_API sk_font_style_t *sk_fontstyle_new(int weight, int width,
...
SK_C_API sk_shader_t *sk_image_make_shader(
    const sk_image_t *image, sk_tile_mode_t tileX, sk_tile_mode_t tileY,
    const sk_sampling_options_t *samplingOptions, const sk_matrix_t *cmatrix);
SK_C_API sk_image_t *sk_image_new_from_encoded(sk_data_t *encoded);
SK_C_API sk_image_t *sk_image_new_raster_data(const sk_image_info_t *cinfo,
                                              sk_data_t *pixels,
                                              size_t rowBytes);
SK_C_API bool sk_image_read_pixels(const sk_image_t *image,
                                   const sk_image_info_t *dstInfo,
...
SK_C_API sk_image_t *sk_image_texture_from_image(gr_direct_context_t *ctx,
                                                 const sk_image_t *image,
                                                 bool mipmapped, bool budgeted);

// ===== Functions from include/core/SkImageFilter.h =====
SK_C_API sk_image_filter_t *sk_imagefilter_new_arithmetic(
    float k1, float k2, float k3, float k4, bool enforcePMColor,
    sk_image_filter_t *background, sk_image_filter_t *foreground,
    const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *sk_imagefilter_new_blur(float sigmaX, float sigmaY,
                                                    sk_tile_mode_t tileMode,
                                                    sk_image_filter_t *input,
                                                    const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *
sk_imagefilter_new_color_filter(sk_color_filter_t *cf, sk_image_filter_t *input,
                                const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *
sk_imagefilter_new_compose(sk_image_filter_t *outer, sk_image_filter_t *inner);
SK_C_API sk_image_filter_t *
sk_imagefilter_new_dilate(int radiusX, int radiusY, sk_image_filter_t *input,
                          const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *sk_imagefilter_new_displacement_map_effect(
    ...
    const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *sk_imagefilter_new_distant_lit_diffuse(
...
SK_C_API sk_image_filter_t *sk_imagefilter_new_distant_lit_specular(
    const sk_point3_t *direction, sk_color_t lightColor, float surfaceScale,
    float ks, float shininess, sk_image_filter_t *input,
    const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *
sk_imagefilter_new_drop_shadow(float dx, float dy, float sigmaX, float sigmaY,
                               sk_color_t color, sk_image_filter_t *input,
                               const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *sk_imagefilter_new_drop_shadow_only(
    float dx, float dy, float sigmaX, float sigmaY, sk_color_t color,
    sk_image_filter_t *input, const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *sk_imagefilter_new_erode(int radiusX, int radiusY,
                                                     sk_image_filter_t *input,
                                                     const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *
...
SK_C_API sk_image_filter_t *sk_imagefilter_new_image_source_default(
    sk_image_t *image, const sk_sampling_options_t *samplingOptions);
SK_C_API sk_image_filter_t *sk_imagefilter_new_magnifier(
    ...
    const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *sk_imagefilter_new_matrix_convolution(
    const sk_isize_t *kernelSize, const float kernel[], float gain, float bias,
    const sk_ipoint_t *kernelOffset, sk_tile_mode_t tileMode,
    bool convolveAlpha, sk_image_filter_t *input, const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *sk_imagefilter_new_matrix_transform(
    const sk_matrix_t *matrix, const sk_sampling_options_t *samplingOptions,
    sk_image_filter_t *input);
SK_C_API sk_image_filter_t *
sk_imagefilter_new_merge(sk_image_filter_t *filters[], int count,
                         const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *
sk_imagefilter_new_offset(float dx, float dy, sk_image_filter_t *input,
                          const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *sk_imagefilter_new_point_lit_diffuse(
    const sk_point3_t *location, sk_color_t lightColor, float surfaceScale,
    float kd, sk_image_filter_t *input, const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *sk_imagefilter_new_point_lit_specular(
    const sk_point3_t *location, sk_color_t lightColor, float surfaceScale,
    float ks, float shininess, sk_image_filter_t *input,
    const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *sk_imagefilter_new_spot_lit_diffuse(
    ...
    const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *sk_imagefilter_new_spot_lit_specular(
    const sk_point3_t *location, const sk_point3_t *target,
    float specularExponent, float cutoffAngle, sk_color_t lightColor,
    float surfaceScale, float ks, float shininess, sk_image_filter_t *input,
    const sk_rect_t *cropRect);
SK_C_API sk_image_filter_t *sk_imagefilter_new_tile(const sk_rect_t *src,
                                                    const sk_rect_t *dst,
                                                    sk_image_filter_t *input);
SK_C_API void sk_imagefilter_unref(sk_image_filter_t *filter);

// ===== Functions from include/core/SkMaskFilter.h =====
SK_C_API sk_mask_filter_t *sk_maskfilter_new_blur_with_flags(sk_blur_style_t,
                                                             float sigma,
                                                             bool respectCTM);
...
SK_C_API sk_blend_mode_t
sk_paint_get_blend_mode_or(sk_paint_t *cpaint, sk_blend_mode_t defaultMode);
...
SK_C_API bool sk_paint_get_fill_path(const sk_paint_t *cpaint,
                                     const sk_path_t *src, sk_path_t *dst,
                                     const sk_rect_t *cullRect, float resScale);
...
SK_C_API void sk_paint_set_imagefilter(sk_paint_t *cpaint,
                                       sk_image_filter_t *cfilter);
SK_C_API void sk_paint_set_maskfilter(sk_paint_t *cpaint,
                                      sk_mask_filter_t *cfilter);
SK_C_API void sk_paint_set_path_effect(sk_paint_t *cpaint,
                                       sk_path_effect_t *effect);
...
SK_C_API void sk_paint_set_stroke_join(sk_paint_t *cpaint,
...
SK_C_API void sk_path_add_path(sk_path_t *cpath, sk_path_t *other,
                               sk_path_add_mode_t add_mode);
SK_C_API void sk_path_add_path_matrix(sk_path_t *cpath, sk_path_t *other,
                                      sk_matrix_t *matrix,
                                      sk_path_add_mode_t add_mode);
SK_C_API void sk_path_add_path_offset(sk_path_t *cpath, sk_path_t *other,
                                      float dx, float dy,
                                      sk_path_add_mode_t add_mode);
SK_C_API void sk_path_add_path_reverse(sk_path_t *cpath, sk_path_t *other);
SK_C_API void sk_path_add_poly(sk_path_t *cpath, const sk_point_t *points,
                               int count, bool close);
SK_C_API void sk_path_add_rect(sk_path_t *cpath, const sk_rect_t *crect,
                               sk_path_direction_t cdir);
SK_C_API void sk_path_add_rounded_rect(sk_path_t *cpath, const sk_rect_t *crect,
                                       float rx, float ry,
                                       sk_path_direction_t cdir);
SK_C_API void sk_path_arc_to(sk_path_t *cpath, float rx, float ry,
                             float xAxisRotate, sk_path_arc_size_t largeArc,
                             sk_path_direction_t sweep, float x, float y);
SK_C_API void sk_path_arc_to_with_oval(sk_path_t *cpath, const sk_rect_t *oval,
                                       float startAngle, float sweepAngle,
                                       bool forceMoveTo);
SK_C_API void sk_path_arc_to_with_points(sk_path_t *cpath, float x1, float y1,
                                         float x2, float y2, float radius);
SK_C_API sk_path_t *sk_path_clone(const sk_path_t *cpath);
SK_C_API void sk_path_close(sk_path_t *cpath);
SK_C_API void sk_path_compute_tight_bounds(const sk_path_t *cpath,
                                           sk_rect_t *crect);
SK_C_API void sk_path_conic_to(sk_path_t *cpath, float x0, float y0, float x1,
                               float y1, float w);
...
SK_C_API void sk_path_cubic_to(sk_path_t *, float x0, float y0, float x1,
                               float y1, float x2, float y2);
SK_C_API void sk_path_delete(sk_path_t *cpath);
SK_C_API void sk_path_get_bounds(const sk_path_t *cpath, sk_rect_t *crect);
SK_C_API int sk_path_get_points(const sk_path_t *cpath, sk_point_t *points,
                                int max);
...
SK_C_API void sk_path_rarc_to(sk_path_t *cpath, float rx, float ry,
                              float xAxisRotate, sk_path_arc_size_t largeArc,
                              sk_path_direction_t sweep, float x, float y);
SK_C_API void sk_path_rconic_to(sk_path_t *cpath, float dx0, float dy0,
                                float dx1, float dy1, float w);
SK_C_API void sk_path_rcubic_to(sk_path_t *cpath, float dx0, float dy0,
...
SK_C_API void sk_path_set_filltype(sk_path_t *cpath,
                                   sk_path_fill_type_t cfilltype);
...
SK_C_API sk_path_effect_t *
sk_path_effect_create_2d_line(float width, const sk_matrix_t *matrix);
SK_C_API sk_path_effect_t *
sk_path_effect_create_2d_path(const sk_matrix_t *matrix, const sk_path_t *path);
SK_C_API sk_path_effect_t *
sk_path_effect_create_compose(sk_path_effect_t *outer, sk_path_effect_t *inner);
...
SK_C_API sk_path_effect_t *
sk_path_effect_create_discrete(float segLength, float deviation,
                               uint32_t seedAssist /*0*/);
SK_C_API sk_path_effect_t *sk_path_effect_create_sum(sk_path_effect_t *first,
...
SK_C_API bool sk_path_op(const sk_path_t *path, const sk_path_t *other,
                         sk_path_op_t op, sk_path_t *result);
SK_C_API bool sk_path_simplify(const sk_path_t *path, sk_path_t *result);
SK_C_API void sk_opbuilder_add(sk_op_builder_t *builder, const sk_path_t *path,
                               sk_path_op_t op);
...
SK_C_API sk_shader_t *
sk_shader_new_perlin_noise_turbulence(float baseFrequencyX,
                                      float baseFrequencyY, int numOctaves,
                                      float seed, const sk_isize_t *tileSize);
SK_C_API sk_shader_t *
sk_shader_new_radial_gradient(const sk_point_t *center, float radius,
                              const sk_color_t colors[], const float colorPos[],
                              int colorCount, sk_tile_mode_t tileMode,
                              const sk_matrix_t *localMatrix);
SK_C_API sk_shader_t *sk_shader_new_sweep_gradient(
...
SK_C_API sk_shader_t *
sk_shader_with_local_matrix(const sk_shader_t *shader,
                            const sk_matrix_t *localMatrix);
...
SK_C_API sk_surface_t *
sk_surface_make_raster_direct(const sk_image_info_t *imageInfo, void *pixels,
                              size_t rowBytes,
                              sk_surface_props_t *surfaceProps);
SK_C_API sk_surface_t *
sk_surface_make_raster_n32_premul(const sk_image_info_t *imageInfo,
                                  sk_surface_props_t *surfaceProps);
SK_C_API sk_surface_t *sk_surface_make_surface(sk_surface_t *surface, int width,
                                               int height);
...
SK_C_API sk_canvas_t *sk_surface_make_canvas_svg(const sk_rect_t *bounds,
                                                 sk_wstream_t *stream,
                                                 uint32_t flags);

SK_C_API sk_surface_t *sk_surface_new_backend_render_target(
...
SK_C_API const sk_text_blob_builder_run_buffer_t *
...
SK_C_API sk_text_blob_t *
sk_textblob_builder_make(sk_text_blob_builder_t *builder);
SK_C_API sk_text_blob_builder_t *sk_textblob_builder_new(void);

SK_C_API void sk_textblob_get_bounds(const sk_text_blob_t *blob,
                                     sk_rect_t *bounds);
SK_C_API int sk_textblob_get_intercepts(const sk_text_blob_t *blob,
                                        const float bounds[2],
                                        float intervals[],
                                        const sk_paint_t *paint);
SK_C_API sk_text_blob_t *
sk_textblob_make_from_text(const void *text, size_t byteLength,
                           const sk_font_t *font, sk_text_encoding_t encoding);
SK_C_API sk_text_blob_t *
sk_textblob_make_from_string(const char *text, const sk_font_t *font,
                             sk_text_encoding_t encoding);
...
SK_C_API const sk_strut_style_t *
sk_paragraph_style_get_strut_style(sk_paragraph_style_t *style);
SK_C_API void sk_paragraph_style_set_strut_style(sk_paragraph_style_t *style,
                                                 sk_strut_style_t *strutStyle);
SK_C_API sk_text_direction_t
sk_paragraph_style_get_text_direction(sk_paragraph_style_t *style);
SK_C_API void
sk_paragraph_style_set_text_direction(sk_paragraph_style_t *style,
                                      sk_text_direction_t direction);
SK_C_API const sk_text_style_t *
sk_paragraph_style_get_text_style(sk_paragraph_style_t *style);
SK_C_API void sk_paragraph_style_set_text_style(sk_paragraph_style_t *style,
                                                sk_text_style_t *textStyle);
SK_C_API sk_text_align_t
sk_paragraph_style_get_text_align(sk_paragraph_style_t *style);
SK_C_API void sk_paragraph_style_set_text_align(sk_paragraph_style_t *style,
                                                sk_text_align_t align);
SK_C_API size_t sk_paragraph_style_get_max_lines(sk_paragraph_style_t *style);
SK_C_API void sk_paragraph_style_set_max_lines(sk_paragraph_style_t *style,
                                               size_t maxLines);
SK_C_API float sk_paragraph_style_get_height(sk_paragraph_style_t *style);
SK_C_API void sk_paragraph_style_set_height(sk_paragraph_style_t *style,
                                            float height);
SK_C_API sk_text_height_behavior_t
sk_paragraph_style_get_text_height_behavior(sk_paragraph_style_t *style);
SK_C_API void
sk_paragraph_style_set_text_height_behavior(sk_paragraph_style_t *style,
...
SK_C_API bool
sk_paragraph_style_get_replace_tab_characters(sk_paragraph_style_t *style);
SK_C_API void
sk_paragraph_style_set_replace_tab_characters(sk_paragraph_style_t *style,
                                              bool replace);
SK_C_API bool
sk_paragraph_style_get_apply_rounding_hack(sk_paragraph_style_t *style);
SK_C_API void
sk_paragraph_style_set_apply_rounding_hack(sk_paragraph_style_t *style,
                                           bool apply);
SK_C_API const char *
sk_paragraph_style_get_ellipsis(sk_paragraph_style_t *style);
SK_C_API void sk_paragraph_style_set_ellipsis(sk_paragraph_style_t *style,
                                              const char *ellipsis,
                                              size_t length);
SK_C_API bool sk_paragraph_style_ellipsized(sk_paragraph_style_t *style);
SK_C_API sk_text_align_t
sk_paragraph_style_effective_align(sk_paragraph_style_t *style);

// strut style
SK_C_API sk_strut_style_t *sk_strut_style_new();
SK_C_API sk_string_t **sk_strut_style_get_font_families(sk_strut_style_t *style,
                                                        size_t *count);
SK_C_API void sk_strut_style_set_font_families(sk_strut_style_t *style,
...
SK_C_API void sk_strut_style_set_leading(sk_strut_style_t *style,
                                         float leading);
SK_C_API bool sk_strut_style_get_strut_enabled(sk_strut_style_t *style);
SK_C_API void sk_strut_style_set_strut_enabled(sk_strut_style_t *style,
                                               bool enabled);
SK_C_API bool sk_strut_style_get_force_strut_height(sk_strut_style_t *style);
SK_C_API void sk_strut_style_set_force_strut_height(sk_strut_style_t *style,
                                                    bool force);
SK_C_API bool sk_strut_style_get_height_override(sk_strut_style_t *style);
SK_C_API void sk_strut_style_set_height_override(sk_strut_style_t *style,
                                                 bool override);
SK_C_API bool sk_strut_style_get_half_leading(sk_strut_style_t *style);
SK_C_API void sk_strut_style_set_half_leading(sk_strut_style_t *style,
                                              bool half);
...
SK_C_API void
sk_font_collection_set_default_font_manager(sk_font_collection_t *collection,
                                            sk_font_mgr_t *fontmgr);
SK_C_API void sk_font_collection_set_default_font_manager2(
    sk_font_collection_t *collection, sk_font_mgr_t *fontmgr,
    sk_string_t **default_family_names);
SK_C_API sk_font_mgr_t *
sk_font_collection_get_fallback_manager(sk_font_collection_t *collection);
SK_C_API void
sk_font_collection_set_asset_font_manager(sk_font_collection_t *collection,
                                          sk_font_mgr_t *fontmgr);
SK_C_API void
sk_font_collection_set_dynamic_font_manager(sk_font_collection_t *collection,
                                            sk_font_mgr_t *fontmgr);
SK_C_API void
sk_font_collection_set_test_font_manager(sk_font_collection_t *collection,
                                         sk_font_mgr_t *fontmgr);
SK_C_API int
sk_font_collection_get_font_managers_count(sk_font_collection_t *collection);
SK_C_API void
sk_font_collection_disable_font_fallback(sk_font_collection_t *collection);
SK_C_API void
sk_font_collection_enable_font_fallback(sk_font_collection_t *collection);
SK_C_API int
sk_font_collection_font_fallback_enabled(sk_font_collection_t *collection);
...
SK_C_API void sk_paragraph_paint(sk_paragraph_t *paragraph, sk_canvas_t *canvas,
                                 float x, float y);
...
SK_C_API sk_position_with_affinity_t
sk_paragraph_get_glyph_position_at_coordinate(sk_paragraph_t *paragraph,
                                              float dx, float dy);
SK_C_API sk_text_box_vector_t sk_paragraph_get_rects_for_range(
...
// paragraph builder
SK_C_API sk_paragraph_builder_t *
sk_paragraph_builder_new(sk_paragraph_style_t *style,
                         sk_font_collection_t *font_collection);
SK_C_API void sk_paragraph_builder_delete(sk_paragraph_builder_t *builder);
SK_C_API void sk_paragraph_builder_push_style(sk_paragraph_builder_t *builder,
                                              sk_text_style_t *style);
SK_C_API void sk_paragraph_builder_pop(sk_paragraph_builder_t *builder);
SK_C_API const sk_text_style_t *
sk_paragraph_builder_peek_style(sk_paragraph_builder_t *builder);
SK_C_API void sk_paragraph_builder_add_text(sk_paragraph_builder_t *builder,
                                            const char *text,
                                            size_t text_length);
SK_C_API void sk_paragraph_builder_add_placeholder(
    sk_paragraph_builder_t *builder,
    sk_paragraph_placeholder_style_t placeholderStyle);
SK_C_API sk_paragraph_t *
sk_paragraph_builder_build(sk_paragraph_builder_t *builder);
SK_C_API sk_span_t
sk_paragraph_builder_get_text(sk_paragraph_builder_t *builder);
SK_C_API const sk_paragraph_style_t *
...
SK_C_API text_range_t
sk_text_line_clusters_with_spaces(sk_text_line_t *text_line);
...
SK_C_API void sk_text_line_format(sk_text_line_t *text_line,
                                  sk_text_align_t align, float max_width);
SK_C_API void sk_text_line_paint(sk_text_line_t *text_line,
                                 sk_paragraph_painter_t *painter, float x,
                                 float y);
SK_C_API void sk_text_line_create_ellipsis(sk_text_line_t *text_line,
                                           float max_width,
...
SK_C_API sk_position_with_affinity_t *
sk_text_line_get_glyph_position_at_coordinate(sk_text_line_t *text_line,
                                              float dx);
SK_C_API sk_line_metrics_t *sk_text_line_get_metrics(sk_text_line_t *text_line);
SK_C_API void sk_text_line_shift_vertically(sk_text_line_t *text_line,
                                            float shift);
...
SK_C_API void sk_text_line_set_ascent_style(sk_text_line_t *text_line,
                                            sk_line_metric_style_t style);
SK_C_API void sk_text_line_set_descent_style(sk_text_line_t *text_line,
                                             sk_line_metric_style_t style);
...
SK_C_API sk_text_style_t *
sk_text_style_clone_for_placeholder(sk_text_style_t *style);
...
SK_C_API void sk_text_style_set_foreground_paint(sk_text_style_t *style,
                                                 sk_paint_t *paint);
...
SK_C_API void sk_text_style_set_background_paint(sk_text_style_t *style,
                                                 sk_paint_t *paint);
...
SK_C_API sk_text_decoration_t
sk_text_style_get_decoration_type(sk_text_style_t *style);
SK_C_API void
sk_text_style_set_decoration_type(sk_text_style_t *style,
                                  sk_text_decoration_t decoration);
SK_C_API sk_text_decoration_mode_t
sk_text_style_get_decoration_mode(sk_text_style_t *style);
SK_C_API void sk_text_style_set_decoration_mode(sk_text_style_t *style,
                                                sk_text_decoration_mode_t mode);
SK_C_API sk_text_decoration_style_t
sk_text_style_get_decoration_style(sk_text_style_t *style);
SK_C_API void
sk_text_style_set_decoration_style(sk_text_style_t *style,
                                   sk_text_decoration_style_t decoration_style);
SK_C_API sk_color_t sk_text_style_get_decoration_color(sk_text_style_t *style);
SK_C_API void sk_text_style_set_decoration_color(sk_text_style_t *style,
                                                 sk_color_t color);
SK_C_API float
sk_text_style_get_decoration_thickness_multiplier(sk_text_style_t *style);
SK_C_API void
sk_text_style_set_decoration_thickness_multiplier(sk_text_style_t *style,
                                                  float multiplier);
...
SK_C_API const sk_fontstyle_t *
sk_text_style_get_fontstyle(sk_text_style_t *style);
SK_C_API void sk_text_style_set_fontstyle(sk_text_style_t *style,
                                          sk_fontstyle_t *fontstyle);
SK_C_API sk_text_baseline_t
sk_text_style_get_text_baseline(sk_text_style_t *style);
SK_C_API void sk_text_style_set_text_baseline(sk_text_style_t *style,
                                              sk_text_baseline_t baseline);
SK_C_API float sk_text_style_get_baseline_shift(sk_text_style_t *style);
SK_C_API void sk_text_style_set_baseline_shift(sk_text_style_t *style,
                                               float shift);
...
SK_C_API void sk_text_style_set_height_override(sk_text_style_t *style,
                                                bool height_override);
SK_C_API bool sk_text_style_get_half_leading(sk_text_style_t *style);
SK_C_API void sk_text_style_set_half_leading(sk_text_style_t *style,
                                             bool half_leading);
SK_C_API float sk_text_style_get_letter_spacing(sk_text_style_t *style);
SK_C_API void sk_text_style_set_letter_spacing(sk_text_style_t *style,
                                               float letter_spacing);
SK_C_API float sk_text_style_get_word_spacing(sk_text_style_t *style);
SK_C_API void sk_text_style_set_word_spacing(sk_text_style_t *style,
                                             float word_spacing);
...
SK_C_API sk_string_t **sk_text_style_get_font_families(sk_text_style_t *style,
                                                       size_t *count);
SK_C_API void sk_text_style_set_font_families(sk_text_style_t *style,
                                              const sk_string_t **fontFamilies,
                                              size_t count);
SK_C_API const sk_string_t *sk_text_style_get_locale(sk_text_style_t *style);
SK_C_API void sk_text_style_set_locale(sk_text_style_t *style,
                                       const sk_string_t *locale);
SK_C_API sk_typeface_t *sk_text_style_get_typeface(sk_text_style_t *style);
SK_C_API void sk_text_style_set_typeface(sk_text_style_t *style,
...
SK_C_API sk_wstream_t *
sk_dynamic_memory_wstream_as_wstream(sk_dynamic_memory_wstream_t *stream);
SK_C_API bool
sk_dynamic_memory_wstream_write(sk_dynamic_memory_wstream_t *stream,
                                const void *buffer, size_t size);
SK_C_API size_t
sk_dynamic_memory_wstream_bytes_written(sk_dynamic_memory_wstream_t *stream);
SK_C_API size_t
sk_dynamic_memory_wstream_read(sk_dynamic_memory_wstream_t *stream,
                               void *buffer, size_t offset, size_t size);
SK_C_API void
sk_dynamic_memory_wstream_delete(sk_dynamic_memory_wstream_t *stream);
...
SK_C_API bool sk_file_wstream_write(sk_file_wstream_t *stream,
                                    const void *buffer, size_t size);
...
SK_C_API sk_canvas_t *sk_document_begin_page(sk_document_t *doc, float width,
                                             float height);
...
SK_C_API sk_document_t *sk_document_make_pdf(sk_wstream_t *stream,
